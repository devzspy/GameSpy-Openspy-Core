-- Load_Z_HO  Load_Z_BH Load_Z_DT Load_Z_EL Load_Z_SM Load_Z_OI Load_Z_LV Load_Z_SR Load_Z_SV Load_Z_SV2 Load_Z_CH Load_Z_DN Load_Z_MA Load_Z_MS Load_Z_AT Load_Z_SZ Load_Z_JA
local int_map_crcs = { -1111044638, -649219893, -1693037822, -1855856107, -83990700, 467992080, -1111384026, 2012637857, 1889251000, 220066192, -1068072566, 654906528, -726637080, -429801657, 2033050259, 1749582439}
...
function ProcessResults(profileid, snapshotdata, done) 
	...
	for i=0,max_players do
		...
		local pct = 100
		if pid > 0 then
			local currentRating = GetPlayerIntValue(gameid, 2, 0, pid, "THAWRating")
			local curHighScore = GetPlayerIntValue(gameid, 2, mapindex, pid, "THAWHighScore")
			local curHighCombo = GetPlayerIntValue(gameid, 2, mapindex, pid, "THAWHighCombo")
			...
			if playerscore > curHighScore then
				SetPlayerIntValue(gameid, 2, mapindex, pid, "THAWHighScore", playerscore)
				SetPlayerIntValue(gameid, 0, mapindex, pid, "TotalHSIncs", totalMapIncreases+1)
				updateRank = 1
			end
			if playercombo > curHighCombo then
				SetPlayerIntValue(gameid, 2, mapindex, pid, "THAWHighCombo", playercombo)
...
--				updateRank = 1
			...
			if updateRank == 1 then
				...
				if currentRating > 3500 then
					currentRating = 3500
				end
				SetPlayerIntValue(gameid, 2, 0, pid, "THAWRating", currentRating)
			end
