...
void Config::handleVarAssignment(std::string theIdentifier) {
	...
	while(aChar != ';') {
		if(aChar == '\"') {
			...
			inet_pton(AF_INET, ipStr.c_str(), &serveraddr);
			var->value.ival =  serveraddr.s_addr;
			rootVariables.push_back(var);			
...
configVar *Config::getRootConfigVar(char *name) {
	std::list<configVar *>::iterator it = rootVariables.begin();
	configVar *var;
	while(it != rootVariables.end()) {
		var = *it;
...
configVar *Config::getRootArray(char *name) {
	std::list<configVar *>::iterator it = rootVariables.begin();
	configVar *var;
	while(it != rootVariables.end()) {
		var = *it;
...
char *Config::getArrayString(configVar *arrayptr, char *name) {
...
int Config::getArrayInt(configVar *arrayptr, char *name) {
	std::list<configVar *> varlist = getArrayVariables(arrayptr);
	std::list<configVar *>::iterator it = varlist.begin();
	configVar *var;
	while(it != varlist.end()) {
...
char *Config::getRootString(char *name) {
	configVar *var;
...
int Config::getRootInteger(char *name) {
	configVar *var;
	if((var = getRootConfigVar(name)) == NULL) {
		return 0;
	}
